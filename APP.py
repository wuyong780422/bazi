...
with tab8:
    if st.button("deepseek 八 字 解 读",use_container_width=True):
        ...
        st.rerun()
# ===================== 【零报错版】吉日模块（100%复刻电脑版） =====================
# 把你 APP.py 里的旧吉日模块，整个删掉，替换成下面这段即可
import streamlit as st
from datetime import datetime, timedelta
import sqlite3

# 【关键修正】：把 sun_star_table 定义在函数外面，避免 NameError
sun_star_table = {
    "甲子": ([6, 15, 24], "未"), "乙丑": ([1, 10, 19, 28], "申"), "丙寅": ([1, 10, 19, 28], "辰"),
    "丁卯": ([8, 17, 28], "申"),
        ...
        if query_btn:
            with st.spinner("正在筛选顶级吉日..."):
                ...
                for i in range(1, 1826):
                    ...
                    # ==============================================
                    # 关键修复：容错处理，就算first_gz为空，也不影响黄道吉日筛选
                    # ==============================================
