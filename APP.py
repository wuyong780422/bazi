        ...
        if can_query and query_btn:
            with st.spinner("正在筛选顶级吉日..."):
                ...
                for i in range(1, 1826):
                    ...
                    m, d = dt.month, dt.day
                    # ===================== 【核心修复】从数据库读取真实干支（和电脑版完全一致） =====================
                    cursor.execute("SELECT 红砂,农历日,年,月建 FROM calendar WHERE 国历 = ? LIMIT 1", (date_str,))
                    res = cursor.fetchone()
                    if not res: continue
                    红砂值, lunar_day, day_nian, day_yue = res
                    # 真实日干支：从数据库年柱取天干，月建取地支（电脑版同源逻辑）
                    day_gan = day_nian[0]
                    day_zhi = day_yue[1]
                    ...
                    fcur = conn.cursor()
                    fcur.execute("SELECT 年 FROM calendar WHERE 国历 = ? LIMIT 1", (first_str,))
                    fres = fcur.fetchone()
                    ...
                    # ===================== 【修复】修灶择日 → 隐藏不修灶 =====================
                    if jiri_type == "修灶择日" and peng == "不修灶":
                        peng = ""
