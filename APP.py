...
with tab8:
    if st.button("deepseek 八 字 解 读",use_container_width=True):
        ...
        # 这里执行解读功能代码
        st.rerun()
# ===================== 独立吉日页面（main逻辑对齐版·12种类型·APP格式输出） =====================
        ...
        if can_query and query_btn:
            with st.spinner("正在筛选顶级吉日..."):
                ...
                def get_jianxing(yue_zhi, day_zhi):
                    ...
                    return jx[idx], JIANXING_TABLE[jx[idx]]
                # 3. 天赦
                ...
                for i in range(1, 1826):
                    ...
                    day_gan = day_ri[0] if len(day_ri)>=2 else ""
                    day_zhi = day_ri[1] if len(day_ri)>=2 else ""
                    day_gz = day_ri

                    # 凶煞过滤（main完整规则）
                    if 红砂值 and 红砂值.strip() == "红砂": continue
                    ...
                    first_gz = cursor.fetchone()[0] if cursor.fetchone() else ""
                    is_sun, sun_tm = False, ""
