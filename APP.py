...
# 顶部标题栏（纯原生组件，无自定义HTML）
col1, col2, col3 = st.columns([1, 3, 1])
with col1:
    if st.button("📱", key="contact_btn"):
        st.session_state.show_wechat = not st.session_state.show_wechat
with col2:
    st.markdown("<h1 style='text-align:center; font-size:24px;'>真命盘专业版</h1>", unsafe_allow_html=True)
with col3:
    st.write("")

# 联系我们（用原生expander实现，稳定无冲突）
if st.session_state.show_wechat:
    with st.expander("📞 联系我们", expanded=True):
        st.image("wechat.png", width=220)
        st.markdown("扫描二维码添加微信")
